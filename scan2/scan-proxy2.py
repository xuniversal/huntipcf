...
def Cek_ip_port(ip_address, port, save_path, active_cache, dead_cache):
    ...
    result = f"{ip_address},{port},Unknown,Unknown"  # Tambahkan placeholder untuk country dan organization
...
def Read_ip_port(start_ip, end_ip, start_port, end_port, max_workers=100):
    ...
    try:
        # Menghasilkan semua IP dalam rentang yang diberikan
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for ip_block in ip_range:
                # Untuk setiap IP dalam blok, kita perlu mengurai dan memindai port
                for ip in ip_block:
